...
DcmDataDictionary::loadDictionary(const char* fileName, OFBool errorIfAbsent)
{
    ...
    if (!OFStandard::fileExists(fileName) || (f = fopen(fileName, "r")) == NULL) {
        ...
        // Change backslashes to forward slashes and remember the
        // last slash position.
        char *slash = filename;
        char *p = filename;
        while(*p) {
        ...
        *(slash+1) = 0;
        search_path.push_back(string(filename) + "dicom.dic");
        search_path.push_back(string(filename) + "lib/dicom.dic");
        search_path.push_back(string(filename) + "../lib/dicom.dic");
#else
        search_path.push_back("/usr/local/lib/dicom.dic");
        search_path.push_back("/usr/local/dicom/lib/dicom.dic");
#endif
        bool found_dict_file = false;
        for (int i=0; i<(int)search_path.size(); i++) {
            //            fprintf(stderr, "Checking %s\n", search_path[i].c_str());
            if (OFStandard::fileExists(search_path[i].c_str())) {
                fprintf(stderr, "Found %s\n", search_path[i].c_str());
                found_dict_file = true;
                fileName = search_path[i].c_str();
                break;
            }
        }
        if (!found_dict_file)
        ...
        if (!found_dict_file && errorIfAbsent) {
            ofConsole.lockCerr() << "DcmDataDictionary: Failed finding dicom.dic file" << endl;
            ofConsole.unlockCerr();
        }
        if (!found_dict_file)
            return OFFalse;
...
DcmDataDictionary::loadExternalDictionaries()
{
    ...
    if ((env == NULL) || (strlen(env) == 0)) {
        ...
        // Hack to search in default locations.
        int ret = loadDictionary("", false);
        fprintf(stderr, "loaddic ret = %d\n", ret);
    }
#endif
